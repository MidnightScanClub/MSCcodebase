function [allends switches] = FCPROCESS_HCP(datafile,tdir,tmasktype,cleanup,roilist,varargin)
...
startdir=pwd;
FDoutlier=0.2; % flag frames over this
GLMoutlierlim=2; % flag frames over this z-score of global signal
GREYoutlierlim=2;
voxdim='222';
switches.WMero=4;
switches.CSFero=1;
restruns = {'rfMRI_REST1_LR';'rfMRI_REST1_RL';'rfMRI_REST2_LR';'rfMRI_REST2_RL'};
[rois x y z] = textread(roilist,'%s%f%f%f');
roimasks = zeros(902629,size(rois,1));
...
for i=1:numsubs
    ...
    for j=1:length(FSfile.dir)
        if isequal(FSfile.vc{j,1},QC(i).vcnum) || isequal([FSfile.vc{j,1} '_2'],[QC(i).vcnum])
            QC(i).fsdir=[FSfile.dir{j,1} '/' FSfile.vc{j,1} '/T1w'];
            found=found+1;
        end
    end
    if found==1
...
for i=1:numsubs
    ...
    QC(i).restruns=restruns;%str2num(tempruns);
    
    % cycle through each BOLD run
    for j=1:numel(QC(i).restruns)
        
        % set original data
        motiontxt{i,j} = [ df.filepath{i,1} '/' QC(i).vcnum '/MNINonLinear/Results/' QC(i).restruns{j} '/Movement_Regressors.txt' ];
        boldimg{i,j} = [ df.filepath{i,1} '/' QC(i).vcnum '/MNINonLinear/Results/' QC(i).restruns{j} '/' QC(i).restruns{j} '_MNI_222.4dfp.img' ];
        boldifh{i,j} = [ df.filepath{i,1} '/' QC(i).vcnum '/MNINonLinear/Results/' QC(i).restruns{j} '/' QC(i).restruns{j} '_MNI_222.4dfp.ifh' ];
        
        anataveimg{i,j} = [ df.filepath{i,1} '/' QC(i).vcnum '/MNINonLinear/Results/' QC(i).restruns{j} '/atlas/' QC(i).restruns{j} '_func_vols_ave.4dfp.img'];
        anataveifh{i,j} = [ df.filepath{i,1} '/' QC(i).vcnum '/MNINonLinear/Results/' QC(i).restruns{j} '/atlas/' QC(i).restruns{j} '_func_vols_ave.4dfp.ifh'];
    
        % check that original data is present
        if ~exist(motiontxt{i,j})
    ...
    mprimg{i,1}=ls([ QC(i).fsdir '/T1w_to_MNI_222.4dfp.img']);
    mprifh{i,1}=ls([ QC(i).fsdir '/T1w_to_MNI_222.4dfp.ifh']);
    mprimg{i,1}(end)=[];
    mprifh{i,1}(end)=[];
...
for i=1:numsubs
    ...
    tmprimg{i,1} = [ QC(i).subatlasdir '/T1w_to_MNI_222.4dfp.img'];
    tmprifh{i,1} = [ QC(i).subatlasdir '/T1w_to_MNI_222.4dfp.ifh'];
    ...
    for j=1:numel(QC(i).restruns)
        ...
        QC(i).subbolddir{j}=[QC(i).subdir '/' QC(i).restruns{j}];
        if ~exist(QC(i).subbolddir{j})
            mkdir(QC(i).subbolddir{j});
        end
        cd(QC(i).subbolddir{j});
        
        tanataveimg{i,1} = [ QC(i).subatlasdir '/' QC(i).restruns{j} '_func_vols_ave_MNI_222.4dfp.img'];
        tanataveifh{i,1} = [ QC(i).subatlasdir '/' QC(i).restruns{j} '_func_vols_ave_MNI_222.4dfp.ifh'];
        system([ 'ln -s ' anataveimg{i,1} ' ' tanataveimg{i,1}]);
        system([ 'ln -s ' anataveifh{i,1} ' ' tanataveifh{i,1}]);
        
        % set symbolic link to original data
        tmotiontxt{i,j} =[ QC(i).subbolddir{j} '/Movement_Regressors.txt' ];
        tboldimg{i,j} = [ QC(i).subbolddir{j} '/' QC(i).restruns{j} '_MNI_222.4dfp.img'];
        tboldifh{i,j} = [ QC(i).subbolddir{j} '/' QC(i).restruns{j} '_MNI_222.4dfp.ifh'];
        system([ 'ln -s ' motiontxt{i,j} ' ' tmotiontxt{i,j}]);
...
% initialize LASTIMG to the 222 image
for i=1:numsubs
    for j=1:numel(QC(i).restruns)
        LASTIMG{i,j,stage}=[ QC(i).subbolddir{j} '/' QC(i).restruns{j} '_MNI_222'];
    end
    cd(QC(i).subdir);
...
if switches.doregression
    ...
    switch switches.regressiontype
        ...
        case {1,9} % freesurfer masks of WM and V
            ...
            % set basic names
            for i=1:numsubs
                QC(i).GLMmaskfile = [ QC(i).fsdir '/brainmask_fs_to_MNI_' voxdim '.4dfp.img'];         
                QC(i).WMmaskfile=[ QC(i).fsdir '/wmparc_cerebralwm_ero' num2str(switches.WMero) '_to_MNI_' voxdim '.4dfp.img'];
                QC(i).CSFmaskfile=[ QC(i).fsdir '/wmparc_CSF_ero' num2str(switches.CSFero) '_to_MNI_' voxdim '.4dfp.img'];
                QC(i).WBmaskfile=[ QC(i).fsdir '/brainmask_fs_to_MNI_' voxdim '.4dfp.img'];
                QC(i).GREYmaskfile=[ QC(i).fsdir '/wmparc_cerebralgm_to_MNI_' voxdim '.4dfp.img'];
            end
            ...
            for i=1:numsubs
                ...
                while nnz(tmpmask)==0
                    ...
                    QC(i).WMmaskfile = [ QC(i).fsdir '/wmparc_cerebralwm_ero' num2str(tmpWMero) '_to_MNI_' voxdim '.4dfp.img'];
                    tmpmask=read_4dfpimg_HCP(QC(i).WMmaskfile);
                    tmpmask=tmpmask & QC(i).DFNDVOXELS;
                end
                QC(i).WMMASK=~~tmpmask;
                ...
                while nnz(tmpmask)==0
                    ...
                    QC(i).CSFmaskfile = [ QC(i).fsdir '/wmparc_cerebralwm_ero' num2str(tmpCSFero) '_to_MNI_' voxdim '.4dfp.img'];
                    tmpmask=read_4dfpimg_HCP(QC(i).CSFmaskfile);
                end
                QC(i).CSFMASK=~~tmpmask;
    ...
    if switches.doregression
        switch switches.regressiontype
            case 0
                ...
                write_4dfpifh_HCP([ QC(i).subseeddir '/usedseeds.4dfp.img'],size(QC(i).seedhold,2),'bigendian',128,128,75,2,2,2);
...
for i=1:numsubs
    for j=1:numel(QC(i).restruns)
        ...
        mvm_temp = load([QC(i).subbolddir{j} '/Movement_Regressors.txt']);
        mvm{i,j} = mvm_temp(:,1:6);
        ddt_mvm{i,j} = mvm_temp(:,7:12);
...
for i=1:numsubs
    ...
    QC(i).runborders=[[1 2 3 4]' tr(i).start];
    %QC(i).runborders=[[1]' tr(i).start];
...
for f=1:numsubs
    ...
    if switches.doregression
        ...
        write_4dfpifh_HCP('beta.4dfp.img',size(zb_out,2),'bigendian',91,109,91,2,2,2);
...
%     end
    ...
    if switches.doblur
        ...
        for j=1:numel(QC(i).restruns)
            ...
            write_4dfpifh_HCP([ LASTIMG{i,j,stage-1} '.4dfp.img'],size(temprun_out,2),'bigendian',91,109,91,2,2,2);
        end
    ...
    write_4dfpifh_HCP([ QC(i).vcnum '_' allends '.4dfp.img'],size(tempimg_out,2),'bigendian',91,109,91,2,2,2);
    ...
    r2=r2(2,1);
    legend({['222 r=' num2str(r1,2) ],['final r=' num2str(r2,2)]});
    hold on;
...
%     write_4dfpifh_HCP(outname,size(tempbold,2),'bigendian',91,109,91,2,2,2);
% end
